...
export function validatorConversorMedidas(unidade: string, numero: number) {
...
export function validatorConversorMoedas(moeda: string) {
  const isMoeda = isString(moeda);

  if (!isMoeda) throw "O parametro moeda é do tipo string.";
}

export function validatorOperacoes(n1: number, n2: number) {
...
export function validatorMedia(array: number[]) {
  const isArr = isArray(array);

  if (!isArr) throw "O parametro array é um array de números.";
}

export function validatorMediaPonderada(array: number[], arrayPesos: number[]) {
  const isArr = isArray(array);
...
export function validatorMath(numero: number) {
  const isNumero = isNumber(numero);

  if (!isNumero) throw "O parametro número é do tipo number.";
}

export function validatorEqSegGrau(a: number, b: number, c: number) {
